...
pub struct InvariantFuzzTestResult {
    ...
    /// asserting via different code paths counts as a single bug. Each entry is the
    /// [`InvariantFuzzError::HandlerAssertion`] variant — uniform with `errors` so callers
    /// can iterate either map with the same failure type.
...
pub(crate) fn can_continue<'a, FEN: FoundryEvmNetwork>(
    ...
    if !call_result.reverted && handlers_succeeded() {
        ...
        if is_assert_failure {
            ...
            // No invariant predicate broke from this handler bug, so `broken` is `None`.
        ...
        if let Some((first_invariant, _)) = failing_invariants.first() {
            ...
            for (invariant, fail_on_revert) in failing_invariants {
                ...
                // Note: handler-side assertion failures are now routed to `broken_handlers`
                // and never reach this branch (`is_assert_failure` is filtered out above).
                // The `BrokenInvariant` arm is kept for compatibility with non-handler-routed
                // assertion paths the upstream branch enabled.

        ...
        if success {
            ...
            if let Some(error) = invariant_result.errors.get(&invariant_contract.anchor().name) {
                let anchor_counterexample = match error {
                    ...
                    // Handler-side assertion bugs are tracked in `handler_errors`, not
                    // `errors`, so they should never be keyed by an invariant name. Treat
                    // defensively: no counterexample to render here.
    ...
    for entry in entries.flatten() {
        ...
        match outcome {
            Ok(outcome) if outcome.anchor_asserted => {
                ...
                // the shorter reproducer. Inlined here because `replayed` is the legacy
                // `(reverter, selector)`-keyed map that `prepare_test` consumes; the unified
                // `handler_site_already_minimal` helper operates on the campaign-internal
                // `FailureKey`-keyed map instead.
